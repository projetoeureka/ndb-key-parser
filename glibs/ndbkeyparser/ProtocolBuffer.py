...
class ExtendableProtocolMessage(ProtocolMessage):
    ...
    def _VerifyExtensionIdentifier(self, extension):
        if extension.containing_cls != self.__class__:
            raise TypeError(
                "Containing type of %s is %s, but not %s."
                % (
                    extension.full_name,
    ...
    def _ListExtensions(self):
        result = [
            ext
            for ext in list(self._extension_fields.keys())
    ...
    def _ExtensionEquals(self, x):
        ...
        for ext in extensions:
            if ext.is_repeated:
                ...
                if self.GetExtension(ext) != x.GetExtension(ext):
                    return False
        return True

    def _OutputExtensionFields(
        self, out, partial, extensions, start_index, end_field_number
    ):
            ...
            if extension.is_repeated:
                ...
                if extension.composite_cls is not None:
                    ...
                    if extension.field_type in _TYPE_TO_DEBUG_STRING:
                        text_value = _TYPE_TO_DEBUG_STRING[extension.field_type](
                            self, value
                        )
                    else:
